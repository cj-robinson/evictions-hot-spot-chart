...
# cleans data and widens to get 2023 -> 2024 diff
...
# gets top 25 neighborhoods and if they increased from 2023
clev_data %>%
  ungroup() %>% 
...
clev_data %>%
  ...
  arrange(`2023`) %>%
  slice_max(`2023`, n = 25) %>% 
  # seperate positive vs negative changes to make diff colors
  ...
  ggplot(aes(x = neigh)) + 
  geom_segment(aes(xend = neigh, y = `2023`, yend = neg_change, color = "Decrease from 2023 to 2024"), 
               arrow = arrow(length = unit(0.02, "npc"))) +
  geom_segment(aes(xend = neigh, y = `2023`, yend = pos_change, color = "Increase from 2023 to 2024"), 
               arrow = arrow(length = unit(0.02, "npc"))) +
  geom_point(aes(y = `2023`, color = "2023 Evictions"), size = 1) +
